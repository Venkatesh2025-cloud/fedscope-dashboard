...
with tab1:
    ...
    if not avail_data.empty:
        fig = px.bar(
            ...
            title="Available Talent by Skill Category", height=450
        )
...
with tab2:
    ...
    if not align_data.empty:
        fig = px.scatter(
            ...
            hover_name="Skill Category", title="Demand vs Supply Ratio per Skill", height=450
        )
        st.plotly_chart(fig, use_container_width=True)
...
with tab5:
    ...
    if not decision_data.empty:
        fig = px.bar(
            ...
            hover_data=["Reasoning"], title="Action Plan by Skill Category", height=450
        )
        st.plotly_chart(fig, use_container_width=True)
