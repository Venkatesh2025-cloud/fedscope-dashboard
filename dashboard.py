...
def load_data(filename, label=None):
    """
    Tries to load a CSV file from local directory.
    If not found, prompts user to upload it manually.
    """
    if os.path.isfile(filename):
        return pd.read_csv(filename)
    else:
        st.warning(f"⚠️ File `{filename}` not found. Please upload it below.")
...
if None in [df_talent, df_demand, df_decision, df_news, df_fedscope]:
    st.stop()  # Don't proceed until all data is loaded
...
with tab1:
    ...
    if not fedscope_data.empty:
        # Occupation Chart
        occ_chart = (
            fedscope_data.groupby("Occupation Title")["Employee Count"]
        ...
        # Agency Chart
        agency_chart = (
            fedscope_data.groupby("Agency Name")["Employee Count"]
            .sum().sort_values(ascending=False).head(10).reset_index()
        )
...
with tab2:
    ...
    if not decision_data.empty:
        ...
        fig_bubble = px.scatter(
            bubble_data, x="city", y="Skill Category", size="Estimated Layoffs",
            title="Layoff Impact by City & Skill", size_max=60
        )
